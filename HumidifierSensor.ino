...
void loop() {
  ...
  if (calibrationRunning && readingRaw == -2 && prevReading > minreading) {
  ...
  else mappedValue = constrain(map(readingRaw, minreading, maxreading, 0, 120), calibrationRunning ? 24 : 0, 120); //map sensor reading to humidifier water level range (0 - 120), 24-120 when calibrating.
  ...
  // For debugging:
#ifdef DEBUG  
  outSerial.print(readingRaw);outSerial.print(" ");outSerial.print((long)mappedValue);outSerial.print(" ");
  outSerial.print(calibrationRunning);outSerial.print(" ");outSerial.print(minreading);outSerial.print(" ");outSerial.println(maxreading);
#endif  
...
#else
  outSerial.print("Packet: ");outSerial.println((long)value);
#endif
}
